  ...
  class DashboardHealthService < BaseService
    ...
    def call
      success(nil, context: {
        ...
        leaderboard:  LeaderboardDistribution.new(season).call
      })
    end
    ...
    private
      ...
      def locale_stats(type, ids, locale)
        present = ids.empty? ? 0 : Translation
          ...
          .count
        { locale: locale, present: present, missing: ids.size - present, pct: pct(present, ids.size) }
      end
